...
Uso:
    python scripts/data/download_history.py
    python scripts/data/download_history.py --ticker AAPL   # solo un ticker
    python scripts/data/download_history.py --resume        # continua download interrotto
"""
import sys
import time
import argparse
...
def init_db(conn: duckdb.DuckDBPyConnection):
    conn.execute("""
        CREATE TABLE IF NOT EXISTS candles (
            ...
            bb_lower     DOUBLE,
            atr          DOUBLE,
            -- Pattern candele (1=presente, 0=assente, -1=ribassista)
            cdl_hammer        INTEGER,
            cdl_engulfing     INTEGER,
            cdl_doji          INTEGER,
            cdl_morning_star  INTEGER,
            cdl_evening_star  INTEGER,
...
# Colonne integer (pattern candele) — default 0; tutte le altre sono float
_INT_COLS = {"cdl_hammer", "cdl_engulfing", "cdl_doji", "cdl_morning_star", "cdl_evening_star"}
...
def main():
    ...
    if not POLYGON_API_KEY or POLYGON_API_KEY == "inserisci_qui_la_tua_key":
        print("✗ API key non configurata in config/.env")
        sys.exit(1)
